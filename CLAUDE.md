# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## 项目概述

这是一个 AI 项目自动化工具包，将 tmux 与 Claude Code 集成，实现自主多 Agent 开发工作流。系统功能包括：

- 在 tmux 会话中启动 Claude Code Agent 进行自主项目开发
- 自调度：Agent 可以使用 `at` 命令安排自己的下次检查时间
- 多 Agent 通信：通过 tmux 消息传递实现跨会话通信
- 自动 git 提交：可配置间隔的定时提交
- 项目规范文件：使用 `project_spec.md` 定义任务和约束

## 架构

```
📌 单项目模式:
╔═══════════════════════════════════════════════════════════════════╗
║                  📦 tmux session: your-project                    ║
╠═══════════════════════╦═══════════════════════╦═══════════════════╣
║      Window 0         ║      Window 1         ║      Window 2     ║
║   ┌─────────────┐     ║   ┌─────────────┐     ║   ┌─────────────┐ ║
║   │ 🤖 Claude   │     ║   │ 💻 Shell    │     ║   │ 🌐 Server   │ ║
║   │   Agent     │     ║   │  (手动操作)  │     ║   │ (Dev)       │ ║
║   └─────────────┘     ║   └─────────────┘     ║   └─────────────┘ ║
╚═══════════════════════╩═══════════════════════╩═══════════════════╝

📌 多项目模式 (Orchestrator 架构):
                ╔═════════════════════════════════╗
                ║    👤 你 (Orchestrator)         ║
                ║    协调 • 监控 • 分配任务        ║
                ╚════════════════╦════════════════╝
                                 │
          ┌──────────────────────┼──────────────────────┐
          │                      │                      │
          ▼                      ▼                      ▼
   ┌─────────────┐        ┌─────────────┐        ┌─────────────┐
   │ 📱 frontend │        │ ⚙️  backend │        │ 📲 mobile   │
   │    Agent    │        │    Agent    │        │    Agent    │
   └─────────────┘        └─────────────┘        └─────────────┘
```

## 文件说明

| 文件 | 用途 |
|------|------|
| `bashrc-ai-automation-v2.sh` | Bash 函数，添加到 ~/.bashrc 中使用，提供核心命令 |
| `project-start-v2.sh` | 独立脚本版本，功能更完整 |
| `AI-Project-Automation-Manual-v2.md` | 用户操作手册（总览） |
| `docs/multi-project-mode.md` | 多项目模式 (Orchestrator) 详细使用手册 |
| `docs/pm-oversight-mode.md` | PM 监督模式详细使用手册 |
| `docs/best-practices.md` | 最佳实践与反模式指南 |
| `.claude/commands/pm-oversight.md` | PM 监督模式的 Claude Code 斜杠命令 |
| `.claude/commands/role-developer.md` | Developer 角色模板 |
| `.claude/commands/role-qa.md` | QA 工程师角色模板 |
| `.claude/commands/role-devops.md` | DevOps 工程师角色模板 |
| `.claude/commands/role-reviewer.md` | 代码审查员角色模板 |
| `.claude/commands/deploy-team.md` | 团队部署命令 |

## 核心命令（source bashrc 文件后可用）

### 基础命令

| 命令 | 说明 |
|------|------|
| `fire <project>` | 快速启动项目（创建 tmux 会话，启动 Claude） |
| `tsc <target> <msg>` | 发送消息到 Claude Code（处理软回车问题） |
| `check-agent [session]` | 查看 Agent 状态 |
| `monitor-snapshot [session]` | 生成格式化监控快照（供 PM 分析） |
| `find-window <名称>` | 按名称查找窗口 |
| `schedule-checkin <分钟> <备注>` | 调度下次检查（自调度功能） |
| `start-auto-commit [session] [分钟]` | 启动自动 git 提交 |
| `list-agents` | 列出所有活跃的 Agent 会话 |
| `broadcast <msg>` | 向所有 Agent 广播消息 |

### 通信协议命令

| 命令 | 说明 |
|------|------|
| `send-status <target> <name> <done> <current>` | 发送状态更新 |
| `send-task <target> <id> <title> <obj> [priority]` | 发送任务分配 |
| `send-bug <target> <severity> <title> ...` | 发送 Bug 报告 |
| `send-ack <target> <task-id>` | 发送任务确认 |
| `send-done <target> <task-id> <summary>` | 发送完成通知 |
| `send-blocked <target> <name> <problem> ...` | 发送阻塞通知 |

### 日志系统命令

| 命令 | 说明 |
|------|------|
| `capture-agent-log <session> [window]` | 捕获 Agent 对话到日志 |
| `view-agent-logs [session]` | 查看今日日志 |
| `end-agent <session> <window> [summary]` | 结束 Agent 并保存日志 |
| `clean-agent-logs [days]` | 清理旧日志（默认保留 7 天） |

### Claude Code 斜杠命令

| 命令 | 说明 |
|------|------|
| `/pm-oversight <项目> SPEC: <文件>` | 启动 PM 监督模式 |
| `/role-developer <任务描述>` | 作为 Developer 角色工作 |
| `/role-qa <测试任务>` | 作为 QA 工程师工作 |
| `/role-devops <运维任务>` | 作为 DevOps 工程师工作 |
| `/role-reviewer <审查内容>` | 作为代码审查员工作 |
| `/deploy-team <项目> [规模]` | 部署 Agent 团队 |

## 配置

环境变量（在 source 之前在 ~/.bashrc 中设置）：
- `CODING_BASE` - 项目目录（默认：`~/Coding`）
- `CLAUDE_CMD` - Claude 命令（默认：`claude`）
- `DEFAULT_DELAY` - 消息发送延迟秒数（默认：`1`）

## 消息发送机制

`tsc` 函数处理 Claude Code 的软回车问题：
```bash
tmux send-keys -t "$target" "$message" C-m
sleep $delay
tmux send-keys -t "$target" Enter  # 需要第二次 Enter
```

## 自调度机制

Agent 可以通过写入 `/tmp/next_check_note_*.txt` 并使用 `at` 命令来安排自己的唤醒时间。系统会在预定时间发送继续工作的消息。

## PM 监督模式

`.claude/commands/pm-oversight.md` 提供 Claude Code 斜杠命令，让一个 Agent 作为项目经理监督另一个 Engineer Agent：

```
/pm-oversight <项目名> SPEC: <规范文件路径>
```

PM Agent 职责：
- 阅读 Spec 文件理解需求
- 定期检查 Engineer 进度
- 监控 Server 窗口日志并反馈错误
- 对照规范验收已完成功能
- 安排下次检查时间
