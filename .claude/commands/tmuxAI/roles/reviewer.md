...
在审查过程中，必须在关键节点输出状态标记，以便 PM 追踪进度。

### 状态标记格式

**审查通过时输出：**
```
[STATUS:DONE] 简要说明审查结果，如：代码审查通过，无阻塞问题
```

**发现必须修复的问题时输出：**
```
[STATUS:ERROR] 发现 N 个必须修复的问题，简要列举
```
...
**审查进度更新时输出（可选）：**
```
[STATUS:PROGRESS] 当前进度，如：已审查 5/8 个文件
```
